...
// POST - Créer une nouvelle inscription à un événement
export async function POST(request: NextRequest) {
    try {
        const body = await request.json()
        console.log("Incoming event body:", body)

        const { firstName, lastName, email, phone, country, event, profile, motivation } = body
        ...
        return NextResponse.json(
            { success: true, message: "Inscription événement envoyée avec succès", id: submission.id },
            { status: 201 }
        )
    } catch (error: any) {
...
// GET - Récupérer toutes les inscriptions aux événements (admin only)
